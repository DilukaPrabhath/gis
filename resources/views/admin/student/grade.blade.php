            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{url('admin/grade/update/store')}}/{{$id}}" method="POST" autocomplete="off" id="regForm" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                            <div class="col-lg-6">
                                <div class="form-group row">
                                    <label class="col-sm-2 col-form-label text-right">Grade Now</label>
                                    <div class="col-sm-10">
                                        <select class="form-control" id="grade_old" name="grade_old">
                                            <option value="">Select</option>
                                            @foreach($grade as $grade_old)

                                            <option value="{{$grade_old->id}}"  {{ $grade_old->id == $data->grade_now ? 'selected' : '' }}>{{$grade_old->grade}}</option>
                                            @endForeach
                                        </select>
                                        @error('grade_old')
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <select class="form-control" id="grade_to_update" name="grade_to_update">
                                            <option value="">Select</option>
                                            @foreach($grade_new as $grade_update)
                                            <option value="{{$grade_update->id}}" >{{$grade_update->grade}}</option>
                                            @endForeach
                                        </select>
