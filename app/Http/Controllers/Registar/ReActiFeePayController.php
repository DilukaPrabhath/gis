...
class ReActiFeePayController extends Controller
{

    public function index(){
        $tbl = ActivityFeePayment::tbl();
        //$tbl = ActivityFeePayment::all();
        return view('registar.activity_payment.index',compact('tbl'));
    }

    public function create(){
        $acti = Activity::all();
        return view('registar.activity_payment.create',compact('acti'));
    }
    ...
    public function student_id(Request $request){
           ...
           public function store(Request $request){
            ...
            Mail::send('registar.emails.mail', $data, function($message) use ($data) {
                $message->to($data['email'])
                ->from('task123test123@gmail.com','test mail')
                ->subject($data['subject']);
              });

            Mail::send('registar.emails.mail', $data2, function($message) use ($data) {
                $message->to($data['email'])
                ->from('task123test123@gmail.com','test mail')
                ->subject($data['subject']);
              });
            ...
            return redirect('registar/activities_payments')->with($notification);
        ...
        public function print($id){
            ...
            return view('registar.activity_payment.print',compact('data','ins'));
        }
